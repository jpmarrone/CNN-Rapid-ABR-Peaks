...
results_name = "ch1_10runs_20260623_144134"
results_folder = os.path.join(outputs_folder, results_name)

csv_path = os.path.join(results_folder, "ch1_all_trial_stats.csv")

figure_folder = os.path.join(results_folder, "ch1_figures_10runs_sd")
...
plt.errorbar(temp["sound_level"], temp["mean_accuracy"], yerr=temp["sd_between_runs"], fmt="o-", linewidth=2.5,
             markersize=11,capsize=4, capthick=2.5, label="Channel 1")
# plt.errorbar(temp["sound_level"], ch2_acc, yerr=ch2_eerbarr, fmt="o-", linewidth=2.5,
...
for target_name in label_names:
    ...
    plt.xlabel("Sound Level (dB)")
    plt.ylabel("Accuracy (%)")
    plt.title(f"Channel 1 - {target_name}", weight="bold")
    plt.xticks(plot_levels)
    plt.ylim(77.5, 102.5)
    plt.grid(False)
    plt.legend(fontsize = 12, loc="lower right")
