...
output_folder = os.path.join(base_folder, "outputs", f"ch2_twoChannel_cnn_amplitudes_{batch_id}")

os.makedirs(output_folder, exist_ok=True)

mat_struct_path = os.path.join(raw_data_folder, "abr_structures_twoChannel.mat")
cnn_model_path = os.path.join(model_folder, "final_ch2_cnn.pth")

amp_output_name = "saf_ch2_abr_amplitudes"
...
for stim, info in stim_info.items():
    ...
    for i, rec in enumerate(recs):
        ...
        adj0 = adjust_prediction(x2_cnn[i], raw_pred[0], mode="peak", window=20, lower_bound = 0)
        adj1 = adjust_prediction(x2_cnn[i], raw_pred[1], mode="trough", window=20, lower_bound=adj0)
        adj2 = adjust_prediction(x2_cnn[i], raw_pred[2], mode="peak", window=20, lower_bound=adj1)
        adj3 = adjust_prediction(x2_cnn[i], raw_pred[3], mode="trough", window=20, lower_bound=adj2)
        adj4 = adjust_prediction(x2_cnn[i], raw_pred[4], mode="peak", window=20, lower_bound=adj3)
        adj5 = adjust_prediction(x2_cnn[i], raw_pred[5], mode="trough", window=20, lower_bound=adj4)
        final = [adj0, adj1, adj2, adj3, adj4, adj5]
    
        #saves user input for updated targets
        if manual_check:
            plt.figure(figsize=(7, 4))
            plt.plot(t_trunc, x1_plot[i], label = "Channel 1", color="gray", linewidth=1.5)
            plt.plot(t_trunc, x2_plot[i], label = "Channel 2", linewidth=2)
            
            for k, j in enumerate(final):
                name = plot_names[k]
                color = marker_colors[k]
                plt.scatter(t_trunc[j], x2_plot[i, j], color=color, s=35, zorder=3)
                plt.text(t_trunc[j] + label_shift, x2_plot[i, j], name, color="black", fontsize=9, fontweight = "bold")
            ...
            if ans == "n":
                ...
                for j in range(1, len(x1_plot[i]) - 1):
                    peak = x2_plot[i, j] > x2_plot[i, j - 1] and x2_plot[i, j] > x2_plot[i, j + 1]
                    trough = x2_plot[i, j] < x2_plot[i, j - 1] and x2_plot[i, j] < x2_plot[i, j + 1]
                    if peak:
                        plt.scatter(t_trunc[j], x2_plot[i, j], color="red", s=35, zorder=3)
                        plt.text(t_trunc[j] + label_shift, x2_plot[i, j], str(j), color="black", fontsize=9, fontweight = "bold")
                    
                    if trough:
                        plt.scatter(t_trunc[j], x2_plot[i, j], color="black", s=35, zorder=3)
                        plt.text(t_trunc[j] + label_shift, x2_plot[i, j], str(j), color="black", fontsize=9, fontweight = "bold")
        ...
        prem_vals = [x2_prem[i, j] for j in full_idx]
        noise_floor = x2_prem[i, noisefloor_range].mean()
...
for p in random.sample(random_plots, n):
    ...
    plt.plot(p["t"], p["ch1"], label = "Channel 1", color="gray", linewidth=1.5)
