...
save_folder = os.path.join(outputs_folder, f"ch1_cv_check_{batch_id}")

os.makedirs(save_folder, exist_ok=True)

training_data_path = os.path.join(training_data_folder, "ch1_cnn_trainingData.pkl")
...
#target adjustment function....movs intital predictions to local extrema
    #enforces an adjustment order
        #because the order of the targets should be w1 peak, w1 trough, w3 peak, w3 trough
        #tries to force that order using lower and upper bounds
            #adjusts wave 3 peak first...because most accurate and easiest to find basically
                #wave 3 peak becomes the anchor point for the rest of the targets
            #wave 3 trough adjusted next...
                #only considers troughs that come after the w3 peak
            #wave 1 peak adjusted next
                #only considers peaks that come before the w3 peak
            #wave 1 trough adjusted next
                #only considers troughs in between the w1 peak and w3 peak
...
label_names = ["W1 Peak", "W1 Trough", "W3 Peak", "W3 Trough"]
    ...
    with torch.no_grad():
        for test_number, (signals, targets) in enumerate(test_loader):
            ...
            if wrong:
                ...
                plt.plot(t, ref_signal, label='Channel 2', linewidth=1.5, color='gray')
                plt.plot(t, prim_signal, label='Channel 1', linewidth=2.5)
...
plt.figure(figsize=(6, 4.5))
plt.plot(plot_levels, acc_overall, marker='o', linewidth=2.5, markersize=11, label="Channel 1")
# plt.plot(plot_levels, acc_ch2, marker='o', linewidth=2.5, markersize=11, label="Channel 2")
...
plt.savefig("overall_accuracy.svg", format="svg", bbox_inches="tight", transparent=True)
...
plt.xlabel("Sound Level (dB)")
plt.ylabel("Accuracy (%)")
plt.title("Channel 1 - Overall", weight='bold')
plt.xticks(plot_levels)
plt.ylim(67, 103)
plt.grid(False)
ax = plt.gca()
...
plt.xlabel("Sound Level (dB)")
plt.ylabel("Accuracy (%)")
plt.title("Channel 1 - Ind. Stimuli", weight='bold')
plt.xticks(plot_levels)
# plt.ylim(89, 101)
plt.grid(False)
plt.legend(fontsize=12, loc="lower right")
...
for j in range(num_targets):
    ...
    plt.xlabel("Sound Level (dB)")
    plt.ylabel("Accuracy (%)")
    plt.title(f"Channel 1 - {label_names[j]}", weight='bold')
    plt.xticks(plot_levels)
    plt.ylim(84, 101)
    plt.grid(False)
    plt.legend(fontsize=12, loc="lower right")
...
for st in stimuli:
    ...
    plt.xlabel("Sound Level (dB)")
    plt.ylabel("Accuracy (%)")
    plt.title(f"Channel 1 - {stim_labels.get(st, st)}", weight="bold")
    plt.xticks(plot_levels)
    # plt.ylim(89, 101)
    plt.grid(False)
    plt.legend(fontsize=12, loc="lower right")
...
for j, tname in enumerate(label_names):
    ...
    plt.title(f"Channel 1 - {tname}", weight="bold")
    plt.xlim(-0.05, 0.85)
    plt.ylim(-0.02, 1.02)
    plt.grid(False)
    ax = plt.gca()
...
for st in stimuli:
    ...
    plt.xlabel("Prediction Adjustment (ms)")
    plt.ylabel("Probability")
    plt.title(f"Channel 1 - {st_name}", weight="bold")
    plt.xlim(-0.05, 0.85)
    plt.ylim(-0.02, 1.02)
    plt.grid(False)
    ax = plt.gca()
...
for st in stimuli:
    ...
    for j, tname in enumerate(label_names):
        ...
        plt.xlabel("Prediction Adjustment (ms)")
        plt.ylabel("Probability")
        plt.title(f"Channel 1 - {st_name} - {tname}", weight="bold")
        plt.xlim(-0.05, 0.85)
        plt.ylim(-0.02, 1.02)
        plt.grid(False)
        ax = plt.gca()
