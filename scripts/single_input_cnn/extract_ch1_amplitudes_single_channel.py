...
output_folder = os.path.join(base_folder, "outputs", f"ch1_singleChannel_cnn_amplitudes_{batch_id}")

os.makedirs(output_folder, exist_ok=True)

mat_struct_path = os.path.join(raw_data_folder, "abr_structures_singleChannel_ch1.mat")
cnn_model_path = os.path.join(model_folder, "final_ch1_cnn_singleChannel.pth")

amp_output_name = "saf_ch1_singleChannel_abr_amplitudes"
...
for stim, info in stim_info.items():
    ...
    for i, rec in enumerate(recs):
        ...
        adj2 = adjust_prediction(x_cnn[i], raw_pred[2], mode="peak", window=20, lower_bound=None, upper_bound=None)
        adj3 = adjust_prediction(x_cnn[i], raw_pred[3], mode="trough", window=20, lower_bound=adj2, upper_bound=None)
        adj0 = adjust_prediction(x_cnn[i], raw_pred[0], mode="peak", window=20, lower_bound=None, upper_bound=adj2)
        adj1 = adjust_prediction(x_cnn[i], raw_pred[1], mode="trough", window=20, lower_bound=adj0, upper_bound=adj2)
        final = [adj0, adj1, adj2, adj3]
        
        if manual_check:
            plt.figure(figsize=(7, 4))
            plt.plot(t_trunc, x_plot[i], label = "Channel 1", linewidth=2)
        ...
        random_plots.append({"title": rec["id"], "t": np.linspace(0, 29.9827, x_prem.shape[1]), "ch1": x_prem[i].copy(), "idx": full_idx})
